                            ...
                            <tbody>
                                @forelse($rekamMedis->detailRekamMedis as $detail)
                                    <tr class="align-middle">
                                        ...
                                        <td><span
                                                class="badge bg-info">{{ $detail->kodeTindakanTerapi->kode ?? '-' }}</span>
                                        </td>
                                        <td>{{ $detail->kodeTindakanTerapi->deskripsi_tindakan_terapi ?? '-' }}</td>
                                        <td>{{ $detail->kodeTindakanTerapi->kategoriKlinis->nama_kategori_klinis ?? '-' }}
                                        </td>
                                        <td>{{ $detail->detail ?? '-' }}</td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="5" class="text-center text-muted">Tidak ada detail tindakan/terapi
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
